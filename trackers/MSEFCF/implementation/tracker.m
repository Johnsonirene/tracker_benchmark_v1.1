...
% 多尺度增强参数
scales = [0.5,1.5]; % 可根据需要调整尺度

% 提取目标区域和背景区域
target_region = x(min_row:max_row, min_col:max_col, :);
background_region = x;

% 计算目标区域的均值和标准差
target_mean = mean(target_region(:));
target_std = std(target_region(:));

% 全局增强只应用于目标区域
enhanced_target_region_global = (target_region - target_mean) * global_enhance_factor + target_mean;

% 初始化多尺度增强后的目标区域
enhanced_target_region_multiscale = enhanced_target_region_global;

% 多尺度增强
for scale_idx = 1:length(scales)
    % 缩放目标区域特征
    scaled_target_region = imresize(target_region, scales(scale_idx));

    % 计算当前尺度下的均值和标准差
    scaled_mean = mean(scaled_target_region(:));
    scaled_std = std(scaled_target_region(:));

    % 将当前尺度的特征映射应用到相应的区域
    scaled_enhanced_feature = (target_region - scaled_mean) * (target_std / scaled_std) + scaled_mean;

    % 累积多尺度增强
    enhanced_target_region_multiscale = enhanced_target_region_multiscale + scaled_enhanced_feature;
end

% 定义多尺度增强和全局增强的权重
alpha_multiscale = 0.7; % 可根据需要调整权重
alpha_global = 0.3;    % 可根据需要调整权重

% 将多尺度增强和全局增强加权结合
combined_enhanced_target_region = alpha_multiscale * enhanced_target_region_multiscale + alpha_global * enhanced_target_region_global;

% 更新x，只更新目标区域，保持背景不变
x(min_row:max_row, min_col:max_col, :) = combined_enhanced_target_region;
shifanfan = x(:,:,1);
% 现在 x 包含了多尺度增强和全局增强加权后的特征
% % 对每个通道进行增强
% enhanced_x = x;
% for d = 1:size(x,3)
%     % 提取目标区域特征
%     target_feature = enhanced_x(:,:,d);
% 
%     % 计算目标区域的均值和标准差
...
% 
%     % 全局增强
%     enhanced_x(:,:,d) = (enhanced_x(:,:,d) - target_mean) * global_enhance_factor + target_mean;
%     global_x=enhanced_x;
%     % 多尺度增强
%     for scale_idx = 1:length(scales)
...
% 
%         % 将当前尺度的特征映射应用到相应的区域
%         scaled_enhanced_feature = (enhanced_x(min_row:max_row, min_col:max_col, d) - scaled_mean) * (target_std / scaled_std) + scaled_mean;
%         % 更新增强后的特征
%         enhanced_x(min_row:max_row, min_col:max_col, d) = scaled_enhanced_feature;
%         alpha=0.3;
%          enhanced_x=alpha*global_x+(1-alpha)* enhanced_x;
%         
%     end
% end
...
% x=enhanced_features;
% shifanfan = x(:,:,1); % 获取增强后的特征
% diff=shifanfan-before;
%         a=size(x,1);
