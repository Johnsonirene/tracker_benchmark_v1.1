...
for frame = 1:num_frames
    ...
    if frame > 1
        pos_pre = pos;
        [xtf, xcf_c, pos, translation_vec, ~, ~, ~] = run_detection(im,pos,sz,random_array,target_sz,currentScaleFactor,features,cos_window,g_f,global_feat_params,use_sz,ky,kx,newton_iterations,featureRatio,Vy,Vx,frame);
        Vy = pos(1) - pos_pre(1);
        Vx = pos(2) - pos_pre(2);
...
% 计算向量 A_vector 的方差
variance = var(A_vector);

    % 生成服从正态分布的随机数矩阵
    channel_matrix = normrnd(mu, variance, [M, N]);
    
    % 将当前通道的矩阵赋值给对应的通道
    random_array(:, :, i) = channel_matrix;
    x(:,:,i)=x(:,:,i)+ random_array(:, :, i);
end
          la=x(:,:,1);
