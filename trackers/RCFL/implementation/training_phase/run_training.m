function [g_f] = run_training(model_xf,xcf,oldR,use_sz,params,yf,small_filter_sz)
    ...
    while (i <= params.admm_iterations)
        ...
        g_f = (((1/(T*mu)) * bsxfun(@times, yf, model_xf))+((params.yta/(T*mu)) * bsxfun(@times, oldR, xcf)) - ((1/mu) * l_f) + h_f) - ...
            bsxfun(@rdivide,(((1/(T*mu)) * bsxfun(@times, u_f, (S_uu .* yf)))+((params.yta/(T*mu)) * bsxfun(@times, u_f, (S_uO .* oldR))) - ((1/mu) * bsxfun(@times, u_f, S_lx)) + (bsxfun(@times, u_f, S_hx))), B);
%         %%% ²»ÓÃsM
