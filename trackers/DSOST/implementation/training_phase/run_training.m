function [g_f] = run_training(model_xf,gcf,xcf,use_sz,params,yf,small_filter_sz)
...
%             bsxfun(@rdivide,(((1/(T*mu)) * bsxfun(@times, u_f, (S_uu .* yf)))-((sqrt(params.yta)/(T*mu)) * bsxfun(@times, S_xx,gcf)) - ((1/mu) * bsxfun(@times, u_f, S_lx)) + (bsxfun(@times, u_f, S_hx))), B);
        %%% ²»ÓÃsM
        g_f = (model_xf .* conj(yf)+params.yta*sum(conj(xcf) .* xcf, 3).*gcf + mu * T * h_f - T * l_f) ./(S_xx0 + params.yta * sum(conj(xcf) .* xcf, 3) + mu * T);
