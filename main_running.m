...
addpath(('D:/ML/vlfeat-master/toolbox'));
vl_setup
 
addpath(('./rstEval'));
% addpath(['./trackers/CACF'])
%  addpath(['./trackers/MRCF'])
seqs=configSeqs; 
...
diary(['./tmp_t/' evalType '.txt']);

numSeq=length(seqs);
numTrk=length(trackers);

finalPath = ['./results/results_uav/results_gamma_' evalType '/'];

if ~exist(finalPath,'dir')
    mkdir(finalPath);
end

tmpRes_path = ['./tmp_t/' evalType '/'];
bSaveImage=0;
